...
> 코드 기반 정확한 참조 문서 · 최초 2026-07-08 · **갱신 2026-07-31 (기준 커밋 `5f02592`)**
> `meeting_minutes_app/` · `web/backend/api/realtime.py` 분석
>
> 기준 커밋을 적어 두는 이유: 날짜만으로는 이 문서가 낡았는지 알 수 없다. 해시가 있으면
> `git log 46577ff..HEAD -- meeting_minutes_app web/backend` 로 그 뒤 무엇이 바뀌었는지
...
**폴백 트리거는 경로마다 다르다 — 위 표의 "어디까지"와 별개다.** 배치 체인은 `예외` **와**
`빈 전사`(HTTP 200 + 빈 텍스트, 제공자의 조용한 실패)를 모두 실패로 보고 다음 제공자로 넘어간다
(`_transcribe_chunk_via_chain`. 정말 발화가 없는 구간까지 순회하지 않도록 `_chunk_is_silent()`로
한 번 갈라낸다). **라이브는 `예외`만** 폴백 조건으로 쓴다 — 빈 전사는 세기만 하고
(`_stt_empty_chunks`) 세그먼트 0으로 끝난 세션의 종료 안내에만 반영한다. 라이브에서 빈 전사를
폴백 조건으로 쓰지 않는 이유는 발화 판정이 거칠기 때문이다: 웹은 RMS 임계값 한 번 넘김
(`realtime.silence_rms`, 무음 드롭 자체도 `realtime.drop_silent_chunks`로 끌 수 있다), CLI는
webrtcvad다. 둘 다 잡음을 발화로 볼 수 있어, 빈 응답을 실패로 단정하면 조용한 회의에서 청크마다
전 제공자를 순회하며 헛돈·지연을 쓴다. 같은 이유로 종료 안내도 원인을 단정하지 않고 저음량과
인식 실패 두 가능성을 함께 알린다.
...
- **런처는 한 번에 하나만 살린다**(`stop_other_instances()`): 어느 런처를 눌러도 앞서 떠
  있던 인스턴스를 끄고 8501 을 넘겨받는다. 락은 폴더 단위라 포터블↔소스를 못 보고 포트로도
  못 찾으므로(랜덤 폴백), 머신 단위 목록(`%LOCALAPPDATA%\MeetingMinutes\instances.json` —
  pid·port·데이터폴더뿐, 비밀 없음)으로 서로를 찾는다. 종료는 `/api/shutdown`(정상 경로)
  우선이고, **진행 중 회의는 409 로 걸려 끄지 않는다**(유일한 예외 — 기존 창을 열어 준다).
  강제 종료 전에는 `_looks_like_ours()` 로 신원을 확인한다(pid 재사용 시 남의 프로세스를
  죽이지 않도록: 포트가 우리 앱으로 응답하거나 그 폴더의 락이 잡혀 있어야 한다).
- 그래서 포트를 옮기는 폴백은 **남의 프로그램**이 8501 을 쥔 경우에만 발동한다. 그때는
  상대 인스턴스의 데이터 폴더까지 안내한다(`describe_port_holder()` → `GET /api/system/info`).
- `--dev` 도 같은 자동 종료를 지나되 **포트는 바꾸지 않고 실패시킨다** —
  `web/frontend/vite.config.ts` 프록시가 `localhost:8501` 하드코딩이라 백엔드만 옮기면 모든
  `/api` 가 조용히 깨진다. 개발 서버 자신은 목록에 등록하지 않는다(`reload=True` 리로더가
  자식을 다시 띄워 '끈 것처럼 보이지만 살아 있는' 상태가 되기 때문).
