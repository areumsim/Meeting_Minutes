...
옵시디언 볼트를 훑어 '아직 처리하지 않은' 녹음을 찾아
    python run_meeting.py process "<파일>" --notify email
...
Windows 작업 스케줄러에 scripts/windows/run_auto_process.bat 를 등록하면 주기적으로 자동 실행됨.
...
def main():
    ...
    for p in new:
        ...
        cmd = [sys.executable, os.path.join(ROOT, "run_meeting.py"), "process", p]
        if NOTIFY:
            cmd += ["--notify", NOTIFY]
