...
function Card({ icon, title, badge, children }: { icon: React.ReactNode; title: string; badge?: React.ReactNode; children: React.ReactNode }) {
  return (
    <section className="bg-surface border border-line rounded-card p-4 md:p-5 mb-3 shadow-sm">
      <h3 className="text-base font-bold mb-2 flex items-center gap-2 text-ink">
        {icon} {title} {badge}
      </h3>
      <div className="text-sm text-ink-2 leading-relaxed space-y-1.5">{children}</div>
    </section>
  );
}
...
function A({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" className="text-ink-2 underline decoration-line-strong hover:text-ink inline-flex items-center gap-0.5">
...
function Badge({ kind }: { kind: "required" | "optional" }) {
  return kind === "required" ? (
    <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-rec-bg text-rec border border-rec">필수</span>
  ) : (
    <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-surface-2 text-ink-3 border border-line">선택</span>
  );
}

// 화면 안내용 알약 모양 텍스트 — 예: [설정] → API 키
function Tag({ children }: { children: React.ReactNode }) {
  return <span className="inline-block font-semibold text-ink bg-hover rounded px-1.5 py-0.5 text-[13px]">{children}</span>;
}
    ...
    <ol className="space-y-2 mt-1">
      {items.map((it, i) => (
        <li key={i} className="flex gap-2.5">
          <span className="shrink-0 w-5 h-5 rounded-full bg-ink text-white text-[11px] font-bold flex items-center justify-center mt-0.5">{i + 1}</span>
...
function Detail({ summary, children, defaultOpen }: { summary: React.ReactNode; children: React.ReactNode; defaultOpen?: boolean }) {
  return (
    <details open={defaultOpen} className="group border border-line rounded-card overflow-hidden">
      <summary className="cursor-pointer list-none flex items-center gap-2 px-3 py-2.5 bg-surface-2/60 hover:bg-surface-2 text-sm font-semibold text-ink select-none">
        <ChevronDown size={15} className="transition-transform group-open:rotate-180 text-ink-3 shrink-0" />
        {summary}
      </summary>
      <div className="px-4 py-3 text-[13px] text-ink-2 space-y-1.5 leading-relaxed">{children}</div>
    </details>
  );
}

function GoBtn({ onClick, children }: { onClick?: () => void; children: React.ReactNode }) {
  return <button onClick={onClick} className="text-ink-2 underline decoration-line-strong hover:text-ink font-medium">{children}</button>;
}
...
export default function Help({ onNavigate }: { onNavigate?: (v: NavTarget) => void }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-1 md:px-0 pb-20 md:pb-0">
      ...
      <p className="text-ink-3 mb-4 text-sm">처음이신가요? 걱정 마세요. <b>딱 한 가지(OpenAI 키)</b>만 넣으면 바로 회의록을 만들 수 있어요. 나머지는 전부 선택입니다.</p>

      {/* 최상단 — 안내 마법사/설정 바로가기 */}
      <div className="flex flex-col sm:flex-row gap-2 mb-2">
        <button
          onClick={openWizard}
          className="flex-1 flex items-center justify-center gap-2 py-3 bg-accent-solid text-white rounded-card font-bold hover:bg-accent-solid-hover transition-all shadow-lg active:scale-[0.98]"
        >
          <Wand2 size={18} /> 설정 마법사 다시 열기
        </button>
        <button
          onClick={() => onNavigate?.("settings")}
          className="flex-1 flex items-center justify-center gap-2 py-3 bg-surface border border-line text-ink rounded-card font-bold hover:bg-surface-2 transition-all active:scale-[0.98]"
        >
          <SettingsIcon size={18} /> 설정 화면 열기
        </button>
      </div>
      <p className="text-xs text-ink-3 mb-4 text-center">설정이 헷갈리면 <b>[설정 마법사 다시 열기]</b>를 누르세요 — 필요한 것만 순서대로 물어봅니다.</p>
      ...
      <Card icon={<KeyRound size={16} />} title="OpenAI 키 발급 방법" badge={<Badge kind="required" />}>
        ...
        <Detail summary="OpenAI 키 발급 단계 (펼치기)">
          <Steps items={[
            ...
            <>화면에 뜬 <span className="font-mono">sk-proj-…</span> 키를 <b>복사</b>합니다. <span className="text-rec">(이 화면을 닫으면 다시 못 보니 지금 복사)</span></>,
        ...
        <div className="mt-2">
          <Detail summary="Claude(Anthropic)로 회의록을 만들고 싶다면 (선택)">
            ...
            <p className="text-ink-3">음성 인식(STT)은 항상 OpenAI를 쓰므로, Claude를 써도 OpenAI 키는 필요합니다.</p>
          </Detail>
        </div>
      </Card>
      ...
      <Card icon={<Sparkles size={16} />} title="회의록 만드는 3가지 방법">
        ...
        <div className="mt-2">
          <Detail summary={<span className="flex items-center gap-1.5"><Mic size={14} /> ② 지금 회의를 실시간으로 받아쓰기 — 녹음</span>}>
            ...
            <p className="text-ink-3">브라우저가 마이크 권한을 물으면 <b>허용</b>하세요. 처음 표시까지 2~6초 걸릴 수 있습니다.</p>
          </Detail>
        </div>
        <div className="mt-2">
      ...
      <Card icon={<FileText size={16} />} title="만든 회의록 열어보고 고치기">
        ...
        <Detail summary="① 문서 탭 — 회의록·요약·스크립트·액션 등" defaultOpen>
          ...
          <p className="text-ink-3">문서가 없는 탭은 자동으로 숨겨집니다. 회의 준비 브리핑 세션은 <b>회의록</b> 탭에만 내용이 있습니다.</p>
        </Detail>
        ...
        <div className="mt-2">
          <Detail summary="③ 메모(지시) 추가해서 다시 만들기 — 재생성" defaultOpen>
            ...
            <Steps items={[
              ...
              <>탭 아래쪽 <b>"노트 반영해 재생성"</b> 칸에 원하는 지시를 적습니다.<br /><span className="text-ink-3">예: "액션 아이템을 표로 정리하고, Q&A 부분을 더 자세히 다뤄주세요."</span></>,
            ...
            <p className="text-ink-3">여러 번 반복해도 됩니다. 원본 전사(스크립트)는 그대로 유지됩니다.</p>
          </Detail>
        </div>
      ...
      <Card icon={<Mail size={16} />} title="회의록 메일로 보내기">
        ...
        <Detail summary="방법 A — 만든 회의록을 그때그때 메일로 (간단)">
          ...
          <p className="text-ink-3">받는 주소를 미리 정해두려면 마법사/설정의 이메일 항목을 채워두면 편합니다.</p>
        </Detail>
        <div className="mt-2">
          <Detail summary="방법 B — 완료되면 자동으로 메일 발송 (설정)">
            ...
            <p className="text-rec">주의: 평소 로그인 비밀번호가 아니라, 메일 서비스 보안설정에서 발급하는 <b>'앱 비밀번호'</b>입니다.</p>
            <p className="text-ink-3">Gmail: 2단계 인증 후 앱 비밀번호 / 네이버: 메일 설정 POP3·SMTP / 아웃룩·회사메일: 계정 보안→앱 암호(막혀 있으면 IT에 SMTP 허용 요청).</p>
          </Detail>
        </div>
      </Card>
      ...
      <Card icon={<ListChecks size={16} />} title="대시보드 — 만든 기록 관리">
        <ul className="list-disc ml-5">
          ...
          <li>상태 아이콘: <span className="text-ok">완료</span> · <span className="text-warn">처리 중</span> · <span className="text-rec">오류</span>. (처리 중이면 자동으로 갱신됩니다)</li>
      ...
      <Card icon={<Mic size={16} />} title="녹음 중 화면에 뜨는 것 · 끄는 법">
        ...
        <div className="space-y-2 mt-1">
          ...
          <Detail summary={<span className="flex items-center gap-1.5"><MessageCircleQuestion size={14} /> 회색 줄 — 페르소나 카드(검증·의견·질문)</span>}>
            ...
            <ul className="list-disc ml-4 space-y-1">
              <li><b>근거</b> — 카드를 누르면 무엇과 대조했는지 함께 보입니다
                (📄 노트 · 🎓 논문 · 🗂 지난 회의 결정·미완료 액션 · 🌐 웹).
          ...
          <Detail summary={<span className="flex items-center gap-1.5"><FileText size={14} /> 회의가 끝난 뒤에는</span>}>
            <ul className="list-disc ml-4 space-y-1">
              <li>회의 중 뜬 <b>중간 정리</b>는 회의 상세의 <Tag>중간 정리</Tag> 탭에 남습니다.
                이건 <b>앱 안에서만</b> 보관됩니다 — 회의록 파일과 달리 메일 첨부나
                노트 폴더 저장에는 들어가지 않습니다(필요하면 그 탭에서 <b>다운로드</b>).</li>
              <li>회의록에는 <b>🔗 관련 노트</b>가 함께 붙고, 확인이 필요한 주장은
                <Tag>사실확인</Tag> 탭으로 정리됩니다(노트 → 논문 → 웹 순서로 대조).</li>
              <li>메일 발송을 켜 두었다면 <b>회의록·요약</b>이 전달됩니다.</li>
            </ul>
          </Detail>
        </div>
      </Card>
      ...
      <Card icon={<Sparkles size={16} />} title="노트 폴더를 연결하면 쓸 수 있는 기능" badge={<Badge kind="optional" />}>
        ...
        <div className="space-y-2 mt-1">
          <Detail summary={<span className="flex items-center gap-1.5"><Sparkles size={14} /> 작동 방식 — 아주 간단히</span>} defaultOpen>
            ...
            <p className="text-ink-3">요약: <b>회의록 생성 → 폴더에 날짜별로 쌓임 → 재빌드로 색인 → 질문하면 관련 노트를 근거로 출처와 함께 답변</b>. 표현이 달라도 뜻으로 찾는 <b>의미 검색</b>이 기본 적용되고, 노트가 많을수록 답이 정확해집니다. (Obsidian 앱 없이 <b>.md 폴더만</b>으로 동작)</p>
          </Detail>
      ...
      <Card icon={<FolderOpen size={16} />} title="내 데이터는 어디에 저장되나요?">
        ...
        <ul className="list-disc ml-5 text-[13px] text-ink-2">
      ...
      <Card icon={<LifeBuoy size={16} />} title="잘 안 될 때 (문제 해결)">
        <div className="space-y-2">
          <Detail summary="회의록 생성이 실패해요">
            <p><Tag>설정</Tag> → API 키에서 <b>연결 테스트</b>가 성공인지 확인하세요. 실패하면 키가 틀렸거나 만료, 또는 OpenAI 결제수단이 없는 경우입니다. (<A href="https://platform.openai.com/account/billing">결제 설정</A>) 상세 화면 상단의 <span className="inline-flex items-center gap-1"><AlertCircle size={13} className="text-rec" /> 빨간 오류 메시지</span>도 원인을 알려줍니다.</p>
          </Detail>
          ...
          <Detail summary="위키 질문·회의 준비·그래프가 결과를 못 찾아요">
            ...
            <p className="text-ink-3">표현이 달라 정답을 못 찾을 때: <b>의미 검색(임베딩)</b>이 기본으로 켜져 있어, 질문과 노트의 말이 달라도 뜻으로 찾아줍니다(OpenAI 키 필요, 재빌드 시 자동 적용). 키가 없으면 글자 매칭만으로 동작하니, 핵심 <b>키워드·고유명사</b>를 넣어 물어보면 더 잘 찾습니다.</p>
          </Detail>
      ...
      */}
      ...
      <p className="text-center text-xs text-ink-3 mt-4">
        헷갈리면 언제든 <button onClick={openWizard} className="text-ink-2 underline font-semibold">설정 마법사</button>를 다시 열어 순서대로 따라 하세요.
      </p>
    </div>
  );
}
