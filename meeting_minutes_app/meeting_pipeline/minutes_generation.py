...
10. **논의된 만큼만 쓸 것.** 회의에서 실제로 다뤄지지 않은 축(계약 조건, 비용, R&R, 일정 등)은
    섹션을 만들지 않는다 — 빈 섹션을 "미정"으로 채우는 것도 금지. 그 축이 실제로 논의됐을 때만 안건으로 분리한다

## 출력 형식 (아래 구조를 따르되, **내용이 있는 섹션만** 출력)
...
(실제 안건 수만큼만 반복 — 형식을 채우려고 안건을 쪼개거나 늘리지 말 것)
...
### 결정 사항
- 명시적으로 합의·확정된 사항만 기재. **확정된 것이 없으면 `확정된 결정 없음` 한 줄만 적는다**
  (위 예시가 2개인 것은 서식 예시일 뿐 개수 하한이 아니다 — 방향성·의견을 결정으로 승격하지 말 것)
...
- **할 일로 명시된 것만 적는다. 없으면 `도출된 액션 없음` 한 줄만 적는다** —
  논의 내용에서 "이렇게 하면 좋겠다" 수준을 액션으로 만들어 내지 말 것

### 섹션 구분
- 주요 섹션 사이에 `---` 구분선 사용

## 길이 기준
- **분량 목표는 없다. 길이는 회의에서 실제로 나온 내용의 양이 정한다.**
  녹음이 길어도 오간 내용이 적으면 회의록은 짧아야 한다 — 분량을 맞추려고
  배경을 추측하거나, 한 발언을 여러 불릿으로 늘리거나, 함의를 덧붙이지 말 것
- 스크립트에 있는 논의·결정·수치·일정은 빠뜨리지 않는다(누락 금지). 반복·중복 발언은 통합
- 근거가 스크립트에 있으면 충실히 적고, 없으면 적지 않는다 — 둘 중 하나이며 그 사이는 없다"""
...
## 핵심 원칙
1. 발표에서 다룬 모든 주제·개념·수치·사례를 누락 없이 반영 — 단, **발표에서 나온 것만**.
   기록을 풍부하게 만들려고 배경 지식을 덧붙이거나 발표자가 하지 않은 설명을 채우지 말 것
...
(**발표·Q&A에서 실제로 제기된 것만** 적는다. 해당 항목이 없으면 그 줄을 생략하고,
 전부 없으면 이 섹션 자체를 생략한다 — 권고사항을 만들어 내는 것은 금지)

- **미해결 질문 / 후속 연구**: Q&A 또는 발표에서 제기됐으나 해결되지 않은 문제점·한계
- **검증 필요 항목**: 발표자가 근거를 제시하지 않은 수치·사실 주장 ([검증 필요] 표시)
- **실무·연구 적용 시 주의사항**: 발표자가 직접 언급한 한계점·전제 조건
...
(발표에서 언급되지 않은 논문·자료를 내부 지식에서 꺼내 추천하지 말 것 — 제목·저자·연도를
 정확히 재현할 수 없어 존재하지 않는 인용이 섞인다. 심화 자료가 필요하면 사용자가 위키
 검색·prep-brief 로 따로 찾는다.)
...
## 길이 기준
- **분량 목표는 없다. 길이는 발표에서 실제로 나온 내용의 양이 정한다.**
  분량을 맞추려고 불릿을 늘리거나, 발표자가 하지 않은 배경 설명·의의·시사점을 덧붙이지 말 것
- 발표 전체 내용을 충실히 반영하되, 반복·중복 설명은 통합 (누락 금지)
- 발표자가 실제로 설명한 배경·맥락·예시·실험 결과는 그대로 남긴다 —
  발표자가 자세히 설명한 소주제는 자세히, 한 줄로 스친 소주제는 한 줄로
- 발표자의 중요한 설명·비유·강조는 직접 인용으로 보존
- 실험 결과·수치·데이터셋 이름·벤치마크 지표는 원문 그대로 기록
- 수식·알고리즘은 발표에서 제시된 범위까지만 적는다(빈칸을 일반론으로 메우지 말 것)"""
...
- **분량 목표는 없다. 길이는 강의에서 실제로 나온 내용의 양이 정한다.**
  분량을 맞추려고 강사가 하지 않은 설명·예시·배경을 채우지 말 것
- 강사가 설명한 이유·예시·논리는 그대로 남긴다 — 강사가 자세히 다룬 개념은 자세히,
  스쳐 지나간 개념은 짧게
- 강의에서 다루지 않은 개념은 "보충"으로도 넣지 않는다"""
...
────────────────────────────────────────
실무/연구 적용 포인트
• **발표자가 직접 언급한** 적용 가능성·검토 필요 사항만
  (발표에 없으면 이 섹션 생략 — 적용 아이디어를 만들어 내지 말 것)

────────────────────────────────────────
후속 학습 자료
• 발표에서 언급된 논문·도구·링크 (저자/연도 포함). 언급이 없으면 섹션 생략
  — 발표에 없는 자료를 내부 지식에서 추천하지 말 것(존재하지 않는 인용이 섞인다)

【작성 원칙】
- 각 섹션은 "무엇이 발표됐고, 어떤 근거·실험이 제시됐는가"를 담는다.
  "왜 중요한가"는 **발표자가 그렇게 말했을 때만** 적는다
- 수치·고유명사·논문명은 원문 그대로 유지
- 발표자가 제시한 근거·예시·맥락은 지우지 않는다(핵심을 제거하는 압축 금지)
- **분량 목표는 없다** — 발표에서 나온 내용의 양이 길이를 정한다"""
...
#: 전사에 남아 있는 STT 환각 표시(text_filters.SUSPECT_MARKER)를 내용으로 쓰지 않게
#: 하는 공통 지시 — 무음 구간 환각은 회의에서 실제로 나온 말이 아니다.
_SUSPECT_HINT = ("⚠ 전사 중 `[불명]` 로 표시된 부분은 음성인식 오류(환각)입니다. "
                 "그 내용은 회의에서 실제로 나온 말이 아니므로 반영하지 말고 무시하세요.\n\n")

#: '누락 금지'와 '분량 채우기 금지'는 한 쌍이어야 한다. 예전에는 앞줄만 있고
#: "짧은 기록이 되지 않도록 하세요"까지 붙어 있어, 알맹이가 적은 녹음에서 모델이
#: 배경·함의·권고를 지어내 분량을 맞추는 것이 **지시된** 상태였다. 게다가 이 블록은
#: memo 템플릿에도 붙는데 그 템플릿은 스스로 "형식적 분량 채우기 금지"라고 써서
#: 같은 프롬프트 안에 상반된 지시가 들어갔다.
_NO_CUT_TAIL = ("단, **분량을 늘리려고 원문에 없는 배경·함의·권고·예시를 만들지 마세요.** "
                "기록의 길이는 실제로 나온 내용의 양이 정합니다 — 내용이 적으면 짧아야 맞습니다.\n\n"
                + _SUSPECT_HINT)
...
#  회의록 / 요약 생성
# ──────────────────────────────────────────────
#: 문서 타입별 필수 섹션. 각 항목은 **동의어 묶음**이고 그중 하나라도 있으면 통과한다.
#: 단일 리터럴이던 시절 "Action Item"만 인정해서, 모델이 헤딩을 `### 액션 아이템`으로만
#: 쓰면(영문 괄호를 떼면) 정상 회의록이 품질 미달로 판정돼 LLM 호출이 한 번 더 나갔다.
#: 표기 흔들림은 품질 문제가 아니다 — 섹션의 **존재**만 본다.
_MINUTES_REQUIRED_HEADERS = {
    "meeting": (("결정 사항", "결정사항", "Decisions"),
                ("Action Item", "액션 아이템", "액션아이템")),
    "seminar": (("핵심 인사이트", "핵심인사이트"),),
    "lecture": (("핵심 정리", "핵심정리"),),
    "memo":    (("핵심 요약", "핵심요약"),),
}
...
def _minutes_is_usable(text: Optional[str], script_len: int, doc_type: str) -> Tuple[bool, str]:
    ...
    **이 게이트는 '분량 하한'이 아니다.** 섹션이 있고 극단적으로 짧지만 않으면 통과다 —
    내용이 적은 회의의 짧은 회의록은 정상이며, 여기서 되돌리면 프롬프트에서 걷어낸
    분량 채우기가 게이트로 되살아난다.
    """
    text = (text or "").strip()
    if not text:
    ...
    missing = [alts[0] for alts in _MINUTES_REQUIRED_HEADERS.get(doc_type, ())
...
def refine_script(
    ...
    system = (
        ...
        "- `[불명]` 표시는 음성인식 환각(무음 구간 오인식)입니다. 표시를 그대로 두고 "
        "내용을 추측해 채우지 마세요.\n"
        "- 같은 문장이 여러 줄 되풀이되면(음성인식 반복 오류) 한 번만 남기세요.\n"
