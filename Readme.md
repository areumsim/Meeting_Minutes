...
```jsonc
{
  ...
  "models": {
    "stt":             "gpt-4o-mini-transcribe",  // STT 기본 모델 (가성비)
    "llm":             "gpt",                     // gpt | claude
    "gpt_model":       "gpt-4o-mini",
    "minutes_model":   "gpt-4o",                  // 회의록 생성 모델 (기본: gpt-4o)
    "summary_model":   "gpt-4o",                  // 요약본 생성 모델 (기본: gpt-4o)
    "claude_model":    "claude-opus-4-6",         // Claude 폴백 모델
    "translate_model": "gpt-4o-mini"
  },
  "realtime": {
    "language":           "en",           // 실시간 기본 언어
...
9. **`refine_script()`** 로 전체 맥락·주제 기반 교정 스크립트 생성 (교정본이 회의록 입력으로 사용됨)
10. `generate_minutes()` / `generate_summary()` 로 회의록 + 요약 생성 (요약은 `.md` + `.txt` 이중 저장)
...
5. 서버 VAD가 발화를 감지하면 전사 이벤트 수신:
   ...
   - `transcription.completed` → 최종 텍스트 확정, 세그먼트 생성
...
10. **`refine_script()`** 로 전체 맥락·주제 기반 교정 스크립트 생성 (교정본이 회의록 입력으로 사용됨)
11. `generate_minutes()` / `generate_summary()` 로 회의록 + 요약 생성 (요약은 `.md` + `.txt` 이중 저장)
...
└── realtime_20250220_143022/               # 세션 서브폴더
    ...
    ├── realtime_20250220_143022_transcript.txt     # 타임스탬프 전사 원문
...
| 옵션 | 설명 | 기본값 |
| --- | --- | --- |
| `--language` | ko / en / auto | config.json (기본: en) |
| `--type` | meeting / seminar / lecture | meeting |
