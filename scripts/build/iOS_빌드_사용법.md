...
| **PC 연결 모드** | 아이폰 → 같은 WiFi의 PC(포터블 배포본) → OpenAI | PC 앱과 **동일한 고품질**(2단계 보정·위키·그래프·워처) | PC에서 `MeetingMinutes.bat` 실행 + LAN 접속 허용 + 같은 WiFi |

- 기본은 단독 모드입니다. [설정] 상단 **"PC 서버 연결"** 카드에 PC 주소를 넣고 "연결"하면 PC 연결 모드가 됩니다.
- PC 연결 모드가 PC 앱과 완전히 같은 경험이라 회의 품질이 중요하면 이 모드를 권장합니다.

> **소리는 어느 모드에서도 "아이폰 마이크"만 들어갑니다.** PC 소리 캡처(온라인 회의
> 상대방 목소리를 직접 받아오는 기능)는 브라우저 화면공유 API 를 쓰는데 iOS WebView 에는
> 그 API 가 없습니다(`Recorder.tsx` 의 `HAS_DISPLAY_MEDIA` — 그래서 [소리 잡는 방법]
> 목록에서 "내 마이크 + 이 PC 소리" 항목 자체가 숨겨집니다). PC 연결 모드에서도 녹음은
> 아이폰이 하고 전사만 PC 가 하므로 같습니다. 줌·팀즈 회의를 통째로 담아야 하면
> **PC 에서 녹음**하세요(그쪽은 [전체 화면] + [시스템 오디오도 공유]로 PC 의 모든 소리를
> 받습니다). 아이폰으로 녹음할 때 상대 목소리는 스피커를 통해서만 들어갑니다 —
> 그때는 [회의실 마이크] 모드가 유리합니다(에코 취소를 끄고 감도를 올립니다).
...
3. **`MeetingMinutes.bat` 을 다시 실행**(바인딩 변경 적용). 먼저 끄지 않아도 됩니다 —
   실행하면 이전 서버를 자동으로 종료하고 8501 을 넘겨받습니다(회의가 진행 중이면
   끄지 않으므로, 그때는 회의가 끝난 뒤 다시 실행하세요).
...
4. 상태가 **"연결됨 (서버 모드)"** 로 바뀌면, 이제 녹음·업로드·회의록이 모두 PC에서 처리됩니다
   (PC 앱과 동일하게 2단계 보정·위키·그래프 적용, OpenAI 키도 PC 것을 사용 → 아이폰엔 키 불필요).
...
| PC 연결 "연결 실패" | ① PC 앱(MeetingMinutes.bat) 켜져 있나 ② LAN 접속 허용 켜고 **재시작**했나 ③ 같은 WiFi인가 ④ 주소·포트(:8501) 정확한가 ⑤ Windows 방화벽 허용했나 |
...
> ⚠ **두 프로파일은 같은 `dist/` 에 쓴다**(vite 기본 outDir 하나). 즉 `npm run ios:sync` 를
> 한 번 돌리면 리포의 `web/frontend/dist` 가 **standalone 번들로 바뀐다**. 그 상태로 PC
> 웹 UI(`webUI_실행.bat`)를 켜면 예전에는 mtime 비교만 해서 "최신이니 스킵"이 되어
> **임의 호스트를 허용하는 CSP 로 PC 앱이 돌았다.** 지금은 두 곳에서 막는다:
> - `run_ui.py::dist_csp_profile()` — dist 가 packaged 가 아니면 mtime 과 무관하게 다시 빌드
> - `build_portable.ps1` 1단계 — 산출물의 `connect-src` 가 `'self'` 가 아니면 빌드 실패
>
> 아이폰 작업 뒤 PC 로 돌아올 때 손으로 할 일은 없다(런처가 알아서 다시 빌드한다).
