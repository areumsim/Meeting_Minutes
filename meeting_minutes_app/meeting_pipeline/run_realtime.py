...
def print_cli_help():
    print("""\
Usage:
  python run_meeting.py realtime                 # interactive realtime launcher
  python run_meeting.py realtime [raw-options]   # pass options to realtime engine

Common raw-options:
  --language ko|en
  --translate
  --type meeting|seminar|lecture
  --topic "회의 주제"
  --mode http|ws|auto
  --recover output/session_YYYYMMDD_HHMMSS.jsonl
  --prev-session output/session_YYYYMMDD_HHMMSS.jsonl
  --email

Examples:
  python run_meeting.py realtime --language ko --topic "주간 회의"
  python run_meeting.py realtime --language en --translate --mode ws
  python run_meeting.py realtime --recover output/session_20260220_143022.jsonl
""")
