...
| [`PRD_회의중_음성브리핑_20260803.md`](PRD_회의중_음성브리핑_20260803.md) | 회의 **중간** 요약 · 이견 지적 · 음성(TTS) 안내 | v0.1 — **미착수.** 트랙 A(중간 요약)는 아래 페르소나 PRD 의 오케스트레이터에 **`summarizer` 페르소나로 합쳐 구현**하기로 결정(2026-08-04, 미결 #3) / 트랙 C(음성) 옵트인 / **트랙 B(회의 중 오류 지적) 보류** |
| [`PRD_회의진행_페르소나에이전트_20260803.md`](PRD_회의진행_페르소나에이전트_20260803.md) | 회의 진행 보조 페르소나 8종 + 참견도 0~5 축 + 하이브리드 계층형(트리아지→생성) | v0.2 — **M0 관찰모드 구현 완료**(2026-08-03~04, 기본 꺼짐). M1(앰비언트 옆 카드) 미착수. M2 는 오탐률 실측 통과가 전제 |
...
| # | 질문 | 왜 막는가 |
|---|---|---|
| 1 | `realtime.two_pass` 기본값 — 켜둔 채 정직하게 2배로 표시할지, 기본 끄고 옵트인으로 돌릴지 | 구현은 어느 쪽이든 같지만 **사용자가 보게 되는 금액이 갈린다** |
...
(마지막 실행 2026-07-31 · 기준 커밋 `acf3c58`)
