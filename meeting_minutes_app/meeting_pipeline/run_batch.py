...
더블클릭하거나 파일을 bat 위에 드래그하거나:
    python run_meeting.py batch [파일1] [파일2] ...
"""
...
def _ask_extra_options() -> list:
    """새 CLI 옵션을 인터랙티브 메뉴에서도 선택할 수 있게 한다."""
    args: list[str] = []
    print(ruler("추가 옵션"))
    print()
    topic = ask("  회의 주제/맥락 (--topic, Enter=없음) >> ").strip()
    if topic:
        args += ["--topic", topic]

    memo = ask("  메모 파일 경로 (--memo, Enter=없음) >> ").strip().strip('"')
    if memo:
        args += ["--memo", memo]

    custom = ask("  LLM 추가 지시 (--custom-prompt, Enter=없음) >> ").strip()
    if custom:
        args += ["--custom-prompt", custom]

    resume = ask("  기존 STT 재사용? (--resume) [y/N] >> ").strip().lower()
    if resume in ("y", "yes", "ㅛ"):
        args.append("--resume")
    else:
        force = ask("  기존 STT가 있어도 새로 전사? (--force-stt) [y/N] >> ").strip().lower()
        if force in ("y", "yes", "ㅛ"):
            args.append("--force-stt")

    no_notify = ask("  이번 실행 알림/메일 생략? (--no-notify) [y/N] >> ").strip().lower()
    if no_notify in ("y", "yes", "ㅛ"):
...
def screen_run_files(files: list):
    ...
    mode = MODES[mode_key]
    extra_options = _ask_extra_options()
    cls()
    print()
    print(c("  " + "═" * W, "title"))
    ...
    ret = _run_batch(files, list(mode["args"]) + extra_options, title)
...
def screen_help():
    ...
    _sections = [
        ("1. 파일 드래그앤드롭", [
            "scripts/windows/run_batch.bat 위에 파일을 드래그하면 자동 감지됩니다.",
        ...
        ("5. 출력 파일", [
            ...
            "  *_summary.md    요약",
            "  *_script.md     전체 텍스트",
            "  *_transcript.md 전체 STT 전사",
            "  wiki_context.json 관련 노트·검증 컨텍스트",
        ]),
        ("6. 추가 옵션", [
            "--resume: 기존 segments.json/transcript.md가 있을 때만 STT 재사용",
            "--force-stt: 기존 STT가 있어도 새로 전사",
            "--topic/--memo/--custom-prompt/--no-notify 지원",
        ]),
        ("7. API 키", [
