...
- 단독 앱 경로는 Obsidian/Vault 검색, 사실검증, Wiki Context/Proposal, 계획 회의 병합을 수행하지 않습니다. 운영 기록과 지식 그래프 축적이 필요한 회의는 PC의 `run_meeting.py batch`, `run_meeting.py realtime`, 또는 서버 `/ws/realtime` 경로를 사용하세요.
...
- 이 정리는 기기 로컬 문서화입니다. 서버 경로에서 제공하는 `Fact Check`, `Wiki Context`, `Wiki Proposal`, Obsidian 발행은 포함되지 않습니다.
