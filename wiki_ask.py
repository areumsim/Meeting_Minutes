...
단독 실행:
    python wiki_ask.py --question "지난 회의에서 결정된 사항은?"
    python wiki_ask.py --question "양자컴퓨팅 현황" --max-notes 7
"""
...
_SYSTEM_PROMPT_TEMPLATE = """당신은 Obsidian 볼트의 지식을 기반으로 정확하게 답변하는 어시스턴트입니다.

규칙:
1. 아래에 제공된 노트 컨텍스트만을 근거로 답변하세요.
2. 모든 주장에는 반드시 [출처: [[노트 제목]]] 형식으로 인용하세요.
3. 제공된 노트에서 확인할 수 없는 내용은 "{unverified}"라고 명시하세요.
4. 두 노트가 서로 상충하는 정보를 담고 있으면 "{conflict}"을 표시하고 두 입장을 모두 제시하세요.
5. 답변은 한국어로 작성하세요.
6. 외부 지식을 사용하지 말고, 제공된 노트에만 의거하세요.
...
class WikiQA:
    ...
    def _gather_context(self, question: str, max_notes: int) -> List[Dict[str, Any]]:
        """인덱서 + Obsidian search 로 관련 노트를 수집한다."""
        seen_titles: set = set()
        results: List[Dict] = []
