...
이 값을 넘는 참견도로 올릴 수 없다(PRD §4). 이 둘은 **3(옆 카드 자동 표시)까지**
열려 있다: 사용자가 원한 기능(회의 중 사실 대조 / 이전 회의와 어긋남 지적)의 담당이
정확히 이 둘인데, 상한 2 로 잠가 두면 설정에서 3 을 골라도 카드가 뜨지 않아
"켰는데 아무것도 안 나온다"가 된다(소유자 결정, 2026-08-05).
**기본값은 여전히 1(관찰)이다** — `config.example.json` 시드도 프리셋도 이 둘을
올리지 않는다. 화면에 내려면 설정에서 **명시적으로** 골라야 하고, 그때 UI 가
오탐률 실측(§15) 전이라는 경고를 함께 보여준다. 4·5(알림음·음성)는 미구현이라
누구에게도 열려 있지 않다.
"""
...
PERSONAS: Dict[str, Persona] = {
    ...
    "fact_checker": Persona(
        ...
        system_prompt=(
            ...
            "**아래에 주어진 근거**(사내 노트·논문·웹 검색 결과·지난 회의 결정)와 "
            "다른 점이 있으면 대조만 하세요. 어느 근거와 대조했는지 함께 밝히세요. "
            "주어진 근거가 없거나 근거로 판단할 수 없으면 개입하지 마세요 — "
            "기억이나 추측으로 사실을 말하지 않습니다. "
            + COMMON_RULES
        ),
        default_level=2,
        model="claude-opus-4-8",
        risk="high",
        hard_cap=3,      # 3=옆 카드까지. 기본은 1(관찰) — 위 모듈 주석
    ),
